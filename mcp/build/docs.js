...
function initializeDocs(server) {
    // Tool to list all available documentation routes
    server.tool("list-docs", "List all available Appwrite ORM documentation routes", {}, async () => {
        const allDocs = {
            baseUrl: BASE_URL,
    ...
    server.tool("get-docs", "Get documentation URL for a specific category", {
        category: zod_1.z.enum(["server", "web"]).describe("The category (server or web)"),
        topic: zod_1.z.string().describe("The topic name (e.g., setup, CRUD, queries, etc.)")
    }, async ({ category, topic }) => {
        const docs = category === "server" ? serverDocs : webDocs;
        const docEntry = docs[topic];
        if (!docEntry) {
